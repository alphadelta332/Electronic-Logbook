...
When private pilot Android distribution is configured, the bundle also carries the
...
The local Android-signing directory also contains the permanent FlightLogX pilot
...
from accepting future updates to an installed pilot app.
...
| Hosted pilot | Docker Desktop, WSL2, Supabase CLI 2.111.0, Firebase CLI 15.28.2, required PostgreSQL 17 client |
...
5. Run `firebase login`, finish Google sign-in in the browser, and confirm
   `firebase projects:list` can read the private pilot project. Firebase login state is
...
- both the development and permanent pilot Android keystores, rewritten paths, protected
