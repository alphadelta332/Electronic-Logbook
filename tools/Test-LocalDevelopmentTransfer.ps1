...
try {
    ...
    Assert-True ($config.Expected.PilotSigningKeystoreFile -eq 'flightlogx-pilot.keystore') 'permanent pilot keystore must have a stable transfer name'
    Assert-True ($config.Expected.PilotSigningCredentialsFile -eq 'flightlogx-pilot-credentials.json') 'permanent pilot credentials must have a stable transfer name'
    Assert-True ($config.Expected.PilotSigningMetadataFile -eq 'flightlogx-pilot-signing.json') 'permanent pilot signing metadata must have a stable transfer name'
    ...
    Assert-True ($entrypointText -match 'Firebase Android pilot config') 'verifier must validate the private Firebase Android config without printing it'
    Assert-True ($entrypointText -match 'Permanent pilot Android signing identity') 'verifier must require the permanent pilot keystore and protected credentials'
    Assert-True ($entrypointText -match "'-storepass:env'.*ELECTRONIC_LOGBOOK_PILOT_VERIFY_STORE_PASSWORD") 'verifier must keep the pilot keystore password out of the keytool command line'
    Assert-True ($entrypointText -match 'Android signing identity was restored incompletely') 'import must reject incomplete permanent pilot signing state'
