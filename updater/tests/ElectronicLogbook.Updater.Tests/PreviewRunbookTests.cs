...
public sealed class PreviewRunbookTests
{
    ...
    public void PreviewRunbookWiresHealthCheckAndLocalEvidence()
    {
        ...
        Assert.Contains("ExpectedAccountEmail", preflightScript, StringComparison.Ordinal);
        Assert.Contains("expected exactly one Auth identity for the invited account", preflightScript, StringComparison.Ordinal);
        Assert.Contains("[long]$counts[1] -ge 1", preflightScript, StringComparison.Ordinal);
        Assert.Contains("ELB_SUPABASE_RLS_DB_URL", preflightScript, StringComparison.Ordinal);
        Assert.Contains("$RlsConnectionString", preflightScript, StringComparison.Ordinal);
        Assert.Contains("must be separate from the hosted Preview database", preflightScript, StringComparison.Ordinal);
        ...
        Assert.Contains("## External Canary Procedure", preflightScript, StringComparison.Ordinal);
        Assert.Contains("### 2\\. Preflight and provision the same email", preflightScript, StringComparison.Ordinal);
        Assert.Contains("`pass` requires", preflightScript, StringComparison.Ordinal);
        Assert.Contains("`failed` applies when data recovery is uncertain", preflightScript, StringComparison.Ordinal);
        Assert.DoesNotContain("## Invitation Process", preflightScript, StringComparison.Ordinal);
        Assert.DoesNotContain("Pass requires:", preflightScript, StringComparison.Ordinal);
