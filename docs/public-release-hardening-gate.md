...
decision and Preview evidence. This guard protects the Android-first FlightLogX Preview
...
- FlightLogX Preview exit decision is `pass` or `pass with issues`;
- Preview evidence covers app-only and workbook-linked users across the full run;
...
- Preview exit decision and evidence links;
- supported launch cohort and geography;
- current hosted-provider limits and selected plan;
- security and privacy changes since the Preview;
